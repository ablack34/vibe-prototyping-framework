...
- **Live-tool collation (Copilot CLI)**: `/vibe-collate` uses the work-iq `ask_work_iq` tool to crawl the tenant directly and return a reviewable **source inventory** (`.copilot-tracking/vibe/{{engagement-kebab}}/m365-source-inventory.md`) the user curates into `sources/`. It complements Path B — collate finds *what artifacts exist*; research tells you *what they mean*. Falls back to the Path B paste-back prompt when the tool isn't available. Feeds `existingDocs`/`priorTranscripts`, not a new gated field.
...
|-------|--------------|-------------------|
| VIBE Preparation | account-team handover, customer's brief (if any), prior transcripts, existing docs, calendar | engagement-brief.md, customer-brief.md, sources/meeting-templates.md (4-week schedule), sources/research/*, m365-source-inventory.md + curated sources/m365/* (via `/vibe-collate`) |
